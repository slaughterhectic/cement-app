    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-heading/60 mt-1">{rows.length} registered · {rows.filter((r) => r.is_active).length} active · {rows.filter((r) => !r.is_active).length} inactive</p>
        </div>
        {canEditTransport() && (
          <button
            type="button"
        ...
        <div className="flex items-center gap-1 rounded-lg border border-card-border bg-surface p-0.5 text-sm">
          {([['all', 'All'], ['active', 'Active'], ['inactive', 'Inactive']] as const).map(([val, label]) => (
            <button key={val} type="button" onClick={() => setStatusFilter(val)}
              className={`px-4 py-1.5 rounded-md font-medium transition-colors ${statusFilter === val ? 'bg-indigo-500 text-white' : 'text-heading/70 hover:bg-card-border/40'}`}>
              {label}
              {val === 'active' && <span className="ml-1 text-xs opacity-70">({rows.filter((r) => r.is_active).length})</span>}
              {val === 'inactive' && <span className="ml-1 text-xs opacity-70">({rows.filter((r) => !r.is_active).length})</span>}
            </button>
          ))}
        </div>
        <button type="button" onClick={() => setShowMonthly((v) => !v)}
          className="rounded-md border border-card-border px-3 py-1.5 text-xs font-medium text-heading/70 hover:bg-surface transition-colors">
          {showMonthly ? 'Hide' : 'Show'} month-wise additions
        </button>
      </div>
      ...
      {showMonthly && (() => {
        ...
        rows.forEach((r) => {
          const d = r.active_since || (r.created_at ? r.created_at.substring(0, 10) : null);
          if (!d) return;
        ...
        return sorted.length > 0 ? (
          <div className="card p-0 overflow-hidden">
            <div className="border-b border-card-border px-4 py-3">
              <p className="text-xs font-semibold uppercase tracking-wider text-heading/60">Trucks added per month</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-card-border bg-surface text-xs font-medium text-heading/60 uppercase tracking-wider text-left">
                  <th className="px-4 py-2">Month</th><th className="px-4 py-2 text-right">Trucks Added</th>
                </tr></thead>
                <tbody>
                  {sorted.map(([ym, count]) => (
                    <tr key={ym} className="border-b border-card-border last:border-0">
                      <td className="px-4 py-2 font-medium text-heading">{ym}</td>
                      <td className="px-4 py-2 text-right">
                        <span className="inline-flex items-center justify-center rounded-full bg-indigo-100 dark:bg-indigo-900/40 px-2.5 py-0.5 text-xs font-medium text-indigo-700 dark:text-indigo-300">{count}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                rows.filter((r) => statusFilter === 'all' || (statusFilter === 'active' ? r.is_active : !r.is_active)).map((row) => (
