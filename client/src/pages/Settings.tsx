  ...
  return (
    <>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-heading/60">{brands.filter(b => b.is_active).length} active brands</p>
      ...
      <div className="card overflow-hidden p-0">
        {loading ? (
          <div className="p-8 text-center text-sm text-heading/50">Loading...</div>
        ) : brands.length === 0 ? (
          <div className="p-8 text-center text-sm text-heading/50">No brands yet. Add one to get started.</div>
        ) : (
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-surface border-b border-card-border">
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-heading/60">Name</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-heading/60">Type</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-heading/60">Manufacturer</th>
                <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-heading/60">Stock (bags)</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-heading/60">Status</th>
                <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-heading/60">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-card-border">
              {brands.map((b) => (
                <tr key={b.id} className={`hover:bg-surface/50 ${b.is_active === 0 ? 'opacity-50' : ''}`}>
                  <td className="px-4 py-3 font-medium text-heading">{b.name}</td>
...
function GodownsPanel() {
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-heading/60">{godowns.length} godown{godowns.length !== 1 ? 's' : ''}</p>
        <button type="button" onClick={openAdd} className="btn-primary">
      ...
      <div className="card overflow-hidden p-0">
        {loading ? (
          ...
          <div className="p-8 text-center text-sm text-heading/50">No godowns yet. Add one to get started.</div>
        ) : (
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-surface border-b border-card-border">
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-heading/60">Name</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-heading/60">Location</th>
                <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-heading/60">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-card-border">
              {godowns.map((g) => (
                <tr key={g.id} className="hover:bg-surface/50">
                  <td className="px-4 py-3 font-medium text-heading">{g.name}</td>
                  <td className="px-4 py-3 text-heading/70">{g.location || '—'}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
...
function BanksPanel() {
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-heading/60">{banks.length} bank{banks.length !== 1 ? 's' : ''} configured</p>
        <button type="button" onClick={openAdd} className="btn-primary">
      ...
      <div className="card overflow-hidden p-0">
        {loading ? (
          ...
          <div className="p-8 text-center text-sm text-heading/50">
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-surface border-b border-card-border">
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-heading/60">Bank Name</th>
                <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-heading/60">Opening Balance (₹)</th>
                <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-heading/60">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-card-border">
              {banks.map((b) => (
                <tr key={b.bank_name} className="hover:bg-surface/50">
                  ...
                  <td className="px-4 py-3 text-right tabular-nums text-heading/80">{formatINR(b.opening_balance)}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button type="button" onClick={() => openEdit(b)} className="rounded p-1.5 text-heading/60 hover:bg-card-border/50" title="Edit">
                        <Pencil className="h-4 w-4" />
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              disabled={!!editing}
            />
            {editing && <p className="mt-1 text-xs text-heading/60">Bank name cannot be changed (it links to transaction history).</p>}
          </div>
          <div>
            ...
            />
            <p className="mt-1 text-xs text-heading/60">Balance before any transactions were recorded in this app.</p>
          </div>
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={() => setModalOpen(false)} className="btn-secondary">Cancel</button>
            <button type="button" onClick={handleSave} disabled={saving} className="btn-primary disabled:opacity-50">
...
function ExpenseCategoriesPanel() {
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-heading/60">{categories.length} categor{categories.length !== 1 ? 'ies' : 'y'}</p>
        <button type="button" onClick={openAdd} className="btn-primary">
      ...
      <div className="card overflow-hidden p-0">
        {loading ? (
          ...
          <div className="p-8 text-center text-sm text-heading/50">No categories yet. Click "Add Category" to get started.</div>
        ) : (
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-surface border-b border-card-border">
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-heading/60">Category Name</th>
                <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-heading/60">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-card-border">
      ...
      <div className="card">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-2 text-xs text-heading/50">
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-heading/60">Manage cement brands, godowns, and data backups</p>
      </div>

      {/* Tabs */}
      <div className="flex flex-wrap gap-1 rounded-lg border border-card-border bg-surface p-1 w-fit">
        {(['brands', 'godowns', 'banks', 'expense-categories', 'transport', 'backup'] as Tab[]).map((t) => (
          <button
            ...
            className={`rounded-md px-4 py-1.5 text-sm font-medium transition-colors ${
              tab === t
                ? 'bg-card text-heading shadow-sm border border-card-border'
