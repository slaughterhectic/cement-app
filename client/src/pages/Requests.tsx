  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        >
          <p className="text-xs font-medium uppercase tracking-wider text-green-600 dark:text-green-400">Completed</p>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-1 text-xs text-heading/50">
                    ...
                    {r.status === 'completed' && r.completed_by_name && (
                      <>
                        <span>·</span>
                        <span className="text-green-600 dark:text-green-400">Done by {r.completed_by_name}</span>
                      </>
                    )}
                  </div>
                  {r.admin_note && (
                  ...
                  <div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                    {r.status === 'pending' ? (
                      ...
                      <button
                        ...
                        className="rounded-lg p-1.5 text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-900/30 transition-colors"
                        title="Reopen"
                      >
                    ...
                    <button
                      ...
                      className="rounded-lg p-1.5 text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors"
                      title="Delete"
                    >
          ...
          >
            <div className="flex items-center justify-between border-b border-card-border px-5 py-4">
              <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${
                detail.status === 'completed' ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300' : 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300'
              }`}>
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              {detail.status === 'completed' && (
                <div className="rounded-lg bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 p-4 flex flex-col gap-1">
                  <div className="flex items-center gap-1.5 text-green-700 dark:text-green-300 text-xs font-semibold">
                    <CheckCircle2 className="h-3.5 w-3.5" />
                    Completed by {detail.completed_by_name}
                  </div>
                  {detail.completed_at && (
                    <p className="text-xs text-green-600 dark:text-green-400">{formatDateTime(detail.completed_at)}</p>
                  )}
                  {detail.admin_note && (
                    <p className="text-sm text-green-800 dark:text-green-200 mt-1 italic">"{detail.admin_note}"</p>
                  )}
                </div>
              )}
                ...
                <div className="flex flex-col gap-2 pt-2 border-t border-card-border">
                  {detail.status === 'pending' ? (
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 rounded-lg border border-amber-300 px-4 py-2.5 text-sm font-medium text-amber-700 dark:text-amber-300 hover:bg-amber-50 dark:hover:bg-amber-900/30 transition-colors"
                    >
                      <RotateCcw className="h-4 w-4" />
                      Reopen
                    </button>
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-lg border border-red-200 dark:border-red-800 px-4 py-2.5 text-sm font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors"
                  >
                    <Trash2 className="h-4 w-4" />
                    Delete Request
                  </button>
