  ...
  if (!dealer) {
    return (
      <div className="space-y-4">
        <Link to="/dealers" className="inline-flex items-center gap-2 text-sm text-heading/70 hover:text-heading">
          <ArrowLeft className="h-4 w-4" /> Back to Dealers
        </Link>
        <p className="text-heading/60">Dealer not found.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <Link to="/dealers" className="mb-2 inline-flex items-center gap-2 text-sm text-heading/70 hover:text-heading">
            <ArrowLeft className="h-4 w-4" /> Back to Dealers
          </Link>
          <h1 className="text-2xl font-bold text-heading">{dealer.name}</h1>
          <div className="mt-1 flex flex-wrap gap-3 text-sm text-heading/60">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-card-border bg-card px-4 py-2.5 text-sm font-medium text-heading/80 hover:bg-surface"
          >
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="card">
          <p className="text-xs font-medium uppercase tracking-wide text-heading/60">Total Charged</p>
          <p className="mt-1 text-2xl font-bold text-heading">{formatINR(summary.totalCharged)}</p>
        </div>
        <div className="card">
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-1 rounded-md border border-card-border bg-card px-2.5 py-1 text-xs font-medium text-heading/80 hover:bg-surface"
          >
            <Plus className="h-3.5 w-3.5" /> Add
          </button>
        </div>
        {subParties.length === 0 ? (
          <p className="text-sm text-heading/60">No sub-parties registered under this dealer yet.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b border-card-border text-left text-xs font-medium uppercase tracking-wide text-heading/60">
              ...
              <tbody className="divide-y divide-gray-50">
                {subParties.map((sp) => (
                  <tr key={sp.id} className="hover:bg-surface">
                    ...
                    <td className="px-3 py-2 text-heading/70">{sp.phone ?? '—'}</td>
                    <td className="px-3 py-2 text-heading/70">{sp.location ?? '—'}</td>
