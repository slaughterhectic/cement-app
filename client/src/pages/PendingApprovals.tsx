...
function EntrySummary({ type, data }: { type: string; data: Record<string, any> }) {
  if (type === 'sale') {
    return (
      <div className="text-sm text-gray-700 space-y-0.5">
        <div><span className="text-gray-500">Party:</span> {data.party_name ?? `ID ${data.party_id}`}</div>
        <div><span className="text-gray-500">Brand:</span> {data.brand_name ?? `ID ${data.brand_id}`}{data.cement_type ? ` (${data.cement_type})` : ''}</div>
        <div><span className="text-gray-500">Bags:</span> {data.bags} @ {formatINR(data.sale_rate)} = <strong>{formatINR(data.bags * data.sale_rate)}</strong></div>
        {data.destination && <div><span className="text-gray-500">Destination:</span> {data.destination}</div>}
        {data.truck_number && <div><span className="text-gray-500">Truck:</span> {data.truck_number}</div>}
        {data.remarks && <div><span className="text-gray-500">Remarks:</span> {data.remarks}</div>}
      </div>
    );
  }
  if (type === 'purchase') {
    return (
      <div className="text-sm text-gray-700 space-y-0.5">
        <div><span className="text-gray-500">Supplier:</span> {data.supplier_name ?? `ID ${data.supplier_id}`}</div>
        <div><span className="text-gray-500">Brand:</span> {data.brand_name ?? `ID ${data.brand_id}`}{data.cement_type ? ` (${data.cement_type})` : ''}</div>
        <div><span className="text-gray-500">Bags:</span> {data.bags} @ {formatINR(data.purchase_rate)}{data.freight_rate ? ` + freight ${formatINR(data.freight_rate)}` : ''} = <strong>{formatINR(data.bags * (Number(data.purchase_rate) + Number(data.freight_rate || 0)))}</strong></div>
        {data.truck_number && <div><span className="text-gray-500">Truck:</span> {data.truck_number}</div>}
        {data.remarks && <div><span className="text-gray-500">Remarks:</span> {data.remarks}</div>}
      </div>
    );
  ...
  if (type === 'payment') {
    return (
      <div className="text-sm text-gray-700 space-y-0.5">
        <div><span className="text-gray-500">Party:</span> {data.party_name ?? `ID ${data.party_id}`}</div>
        <div><span className="text-gray-500">Amount:</span> <strong>{formatINR(data.amount)}</strong> ({data.mode})</div>
        <div><span className="text-gray-500">Direction:</span> {data.direction === 'pay' ? 'Pay to supplier' : 'Receive from customer'}</div>
        {data.bank_name && <div><span className="text-gray-500">Bank:</span> {data.bank_name}</div>}
        {data.remarks && <div><span className="text-gray-500">Remarks:</span> {data.remarks}</div>}
      </div>
    );
  }
  if (type === 'expense') {
    return (
      <div className="text-sm text-gray-700 space-y-0.5">
        <div><span className="text-gray-500">Description:</span> {data.description}</div>
        <div><span className="text-gray-500">Amount:</span> <strong>{formatINR(data.amount)}</strong> ({data.mode})</div>
        {data.category && <div><span className="text-gray-500">Category:</span> {data.category}</div>}
        {data.bank_name && <div><span className="text-gray-500">Bank:</span> {data.bank_name}</div>}
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {(
          [
            { key: 'all',      label: 'Total',    count: rows.length,    color: 'text-gray-600',   ring: 'ring-gray-400' },
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      {(isAdmin || entry.status === 'pending') && (
                        <button
                          ...
                          className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-red-400 transition-colors"
                          title="Remove"
                        >
          ...
          <div className="flex min-h-full items-center justify-center px-4 py-6">
            <div className="w-full max-w-md rounded-xl bg-white shadow-2xl">
              <div className="flex items-center justify-between border-b border-card-border px-5 py-4">
                ...
                <button type="button" onClick={() => setActionTarget(null)} className="rounded-lg p-1.5 hover:bg-gray-100">
              ...
              <form onSubmit={handleAction} className="p-5 flex flex-col gap-4">
                ...
                <div className="rounded-lg bg-gray-50 border border-card-border p-3">
                  ...
                  <p className="text-xs text-gray-400 mt-2">By {actionTarget.entry.created_by_name} · {formatDateTime(actionTarget.entry.created_at)}</p>
                </div>
                ...
                <div className="flex justify-end gap-3 pt-1">
                  <button type="button" onClick={() => setActionTarget(null)} className="rounded-lg border border-card-border px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                    Cancel
                  </button>
