...
function computeTripFields(body: any) {
  ...
  const total_freight = (quantity * freight_rate) + loading_charge + unloading_charge;
  const net_freight = total_freight - advance_deduction - toll_expense;
  const net_profit = net_freight - diesel_amount - driver_payment - miscellaneous;
  const total_km = (odometer_end !== null && odometer_start !== null) ? odometer_end - odometer_start : 0;
...
router.get('/', async (req, res) => {
  try {
    ...
    let sql = `
      SELECT tt.*, t.truck_number, d.name as driver_name
      FROM truck_trips tt
...
router.post('/', async (req, res) => {
  try {
    const { date, truck_id, driver_id, material_name, load_from, billed_party, billed_destination, transport_name, diesel_from, remarks } = req.body;
    ...
    const row = await getOne(
      `INSERT INTO truck_trips (
        date, truck_id, driver_id, material_name, quantity, load_from, billed_party, billed_destination, transport_name,
        freight_rate, loading_charge, unloading_charge,
        advance_litres, advance_rate, advance_deduction,
        toll_expense, diesel_litres, diesel_rate, diesel_amount, diesel_from,
        driver_payment, miscellaneous,
        odometer_start, odometer_end, total_km,
      ...
      [
        date, truck_id, driver_id || null, material_name || null, c.quantity, load_from || null,
        billed_party || null, billed_destination || null, transport_name || null,
        c.freight_rate, c.loading_charge, c.unloading_charge,
        c.advance_litres, c.advance_rate, c.advance_deduction,
        c.toll_expense, c.diesel_litres, c.diesel_rate, c.diesel_amount, diesel_from || null,
  ...
  try {
    ...
    const row = await getOne(
      `UPDATE truck_trips SET
        date=$1, truck_id=$2, driver_id=$3, material_name=$4, quantity=$5, load_from=$6,
        billed_party=$7, billed_destination=$8, transport_name=$9,
        freight_rate=$10, loading_charge=$11, unloading_charge=$12,
        advance_litres=$13, advance_rate=$14, advance_deduction=$15,
        toll_expense=$16, diesel_litres=$17, diesel_rate=$18, diesel_amount=$19, diesel_from=$20,
        driver_payment=$21, miscellaneous=$22,
        odometer_start=$23, odometer_end=$24, total_km=$25,
        total_freight=$26, net_freight=$27, net_profit=$28, remarks=$29
